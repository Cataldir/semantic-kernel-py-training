...
class AIServiceSelectorCorrected(AIServiceSelector):
    ...
    def select_ai_service(
        ...
        for service_id, settings in execution_settings_dict.items():
            service = kernel.get_service(getattr(settings, service_id, None), type=(TextCompletionClientBase, ChatCompletionClientBase))
            if service:
